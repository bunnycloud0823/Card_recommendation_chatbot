...
def extract_card_name_by_id(text, card_id):
    ...
    pattern = rf"([\w가-힣A-Za-z\s]+)\s*카드ID\s*:\s*{card_id}import streamlit as st
import json
import os
import re
import random
...
def show_card_details(card_ids, full_response_text=None):
    for cid in card_ids:
        ...
        st.markdown(f"[{card_name} 카드 신청 링크 열기]({apply_url})", unsafe_allow_html=True)
        st.write("---")
...
with col1:
    age_group = st.radio("연령대", ["10대", "20대", "30대", "40대", "50대 이상"], index=0)
with col2:
    occupation = st.radio("직업", ["학생", "직장인", "취업 준비생", "기타"], index=0)

user_name = st.text_input("닉네임을 입력하세요:", "")

user_info = {"name": user_name or "익명", "age_group": age_group, "occupation": occupation}
    ...
    if st.session_state["messages"][-1]["role"] != "assistant":
        with st.chat_message("assistant"):
            try:
                ...
                # 중복 출력 방지를 위해 여기서는 화면에 다시 출력하지 않음
